...
import com.travel.dx.godaxing.modules.home.dao.HomeTogetherDao;
...
public class TogetherHomeFragment extends BaseFragment {
    ...
    @Subscribe
    public void UpdatePage(PlayTogeInfo info) {
        if(isloadData){
            page++;
            loadData();
        }
    ...
    protected void init() {
        ...
        adapter = new CommonAdapter<HomeDetailsInfo.DataBean>(getActivity(), list,
                R.layout.adapter_home_together_info) {
            @Override
            public void fillData(ViewHolder helper, int position, HomeDetailsInfo.DataBean item) {
                // TODO 填充数据
                helper.setText(R.id.home_together_name,item.getName());// 给一个Item中的TextView设置数据
                double distance =  item.getDistance() / 1000.00;
                DecimalFormat df=new DecimalFormat(".##");
                String result=df.format(distance);
                helper.setText(R.id.home_together_distance, result + "km");
                if (!item.getPrice().equals("null")) {
                    helper.setText(R.id.home_together_price, "￥" + item.getPrice());
                    TextView oldPrice = helper.getView(R.id.home_together_price_after);
                    oldPrice.getPaint().setAntiAlias(true);//抗锯齿
                    oldPrice.getPaint().setFlags(Paint. STRIKE_THRU_TEXT_FLAG|Paint.ANTI_ALIAS_FLAG ); //中间横线
                    helper.setText(R.id.home_together_price_after, "￥" +item.getOriginalprice());
                } else {
                    helper.setText(R.id.home_together_price, "免费");
                    helper.setText(R.id.home_together_price_after,"");
                }

                helper.setImageByUrl(R.id.home_together_iv, item.getImg());// 加载一张网络图片
    ...
    protected void loadData() {
        ...
        HomeTogetherDao.requestTogetherList(page, new BaseCallBack() {
            @Override
    ...
    protected int setLayoutId() {
        return R.layout.layout_home_together;
    }
    @Override
